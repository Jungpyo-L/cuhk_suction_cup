...
from cuhk_suction_cup.srv import *
...
def appendDataPoint(topic, msg):
    ...
    for attributName in list(listOfAttributesInTopic[topic].keys()):
        try:
            ...
            if (
                listOfAttributesInTopic[topic][attributName] > 1
    ...
    if hasattr(msg, "__slots__"):
        ...
        for tempSlotName in thisSlotList:
            if tempSlotName != "header":
                updateListofAttribute(
                    topic,
                    getattr(msg, tempSlotName),
                    attributeName + "." + tempSlotName,
                )

    elif isinstance(
        msg, numbers.Number
    ):  # check if the attributed msg is single Number
        if topic not in listOfAttributesInTopic:
            listOfAttributesInTopic[topic] = {}

        listOfAttributesInTopic[topic].update(
            {attributeName[1:]: 1}
        )  # remove the front dot in the attribute name
        return

    elif (isinstance(msg, list) or isinstance(msg, tuple)) and (
        bool(msg) and isinstance(msg[0], numbers.Number)
    ):  # check if the attributed msg is List
        if topic not in listOfAttributesInTopic:
            listOfAttributesInTopic[topic] = {}

        listOfAttributesInTopic[topic].update(
            {attributeName[1:]: len(msg)}
        )  # remove the front dot in the attribute name
        return
    return
...
def setLoggingState(request):
    ...
    if desiredState == True:
        ...
        if isLoggingEnabled != desiredState:
            ...
            # loadConfigFile(os.path.expanduser('~') + '/CUHK_catkin_ws/src/cuhk_suction_cup/config/TopicsList.txt')
            ...
            for topicName in listOfTopics:
                ...
                output_file_name[topicName] = (
                    "/tmp/dataLog_" + currentTimeStr + "_" + topicName[1:] + ".csv"
                )
