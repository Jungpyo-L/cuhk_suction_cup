...
class Crop_Filter(object):
    ...
    def smooth_trajectory_1d(self, trajectory_1d):
        ...
        if right_ind < len(trajectory_1d) - 1:
            right_val = trajectory_1d[right_ind] * np.ones(
                len(trajectory_1d) - 1 - right_ind
            )
...
class Smooth_Filter(object):
    ...
    def _median_filter(self, trajectory, kernel_size):
        ...
        for ii in range(dim):
            filt_traj[:, ii] = signal.medfilt(
                trajectory[:, ii], kernel_size=kernel_size
            )
    ...
    def _kalman_filter(self, trajectory):
        ...
        self.kf = pykalman.KalmanFilter(
            n_dim_obs=dim, n_dim_state=dim, initial_state_mean=trajectory[0]
        )
    ...
    def _ma_filter(self, trajectory, kernel_size):
        ...
        for ii in range(dim):
            a = trajectory[:, ii]
            out0 = (
                np.convolve(a, np.ones(kernel_size, dtype=int), "valid") / kernel_size
            )
...
class Motion_Sampler(object):
    ...
    def __init__(
        ...
        min_time_step=30,
        interpolator="pchip",
        interpolate_ratio=[3, 2, 1],
        interpolate_threshold=[50, 100],
    ):
        """
